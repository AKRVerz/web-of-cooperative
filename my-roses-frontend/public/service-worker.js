...
const UIStrings = {
  ...
  explanationOutOfScope:
    'This origin has one or more service workers, however the page ' +
  ...
  explanationNoManifest:
    'This page is controlled by a service worker, however ' +
...
class ServiceWorker extends Audit {
  ...
  static audit(artifacts) {
    ...
    const versionsForOrigin = ServiceWorker.getVersionsForOrigin(
      versions,
      pageUrl
    );
    ...
    const serviceWorkerUrls = ServiceWorker.getControllingServiceWorker(
      versionsForOrigin,
      registrations,
      pageUrl
    );
    if (!serviceWorkerUrls) {
      return {
        score: 0,
        explanation: str_(UIStrings.explanationOutOfScope, {
    ...
    const startUrlFailure = ServiceWorker.checkStartUrl(
