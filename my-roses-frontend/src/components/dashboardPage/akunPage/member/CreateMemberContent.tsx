...
const CreateMemberContent: React.FC<Props> = ({ createUser }) => {
  ...
  const create = async (value: Koperasi.Resource.Create['users']) => {
    ...
    try {
      ...
      return setTimeout(() => {
        Router.push('/dashboard/akun/members');
      }, 3000);
    } catch (e) {
      ...
      <DashboardContainer overflow={'auto'}>
        <Flex p={5} flexDirection={'column'} height={'100%'}>
          <Text fontFamily={'Poppins'} fontSize={'1.45rem'} py={3}>
            Formulir Pembuatan Akun Member
          </Text>
          <Formik
            initialValues={{
              email: '',
...
export default connector(CreateMemberContent);
